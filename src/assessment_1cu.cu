//****************************************************
// This is a version of assessment1 to run on NVIDIA
// GPU by employing Cuda. Build the cpp version to use
// CPU rendering.
//****************************************************
...
__global__ void get_light(const vec3 dir_list[], const vec3 c_list[], const int light_n, vec3 dir, vec3 c){
        ...
        for (int j = 0; j < Height_global; j++) {
            ...
            if (dist <= radius) {
                ...
                for(int a=0; a<10; a++){
                    ...
                    vec3 l;
                    vec3 cl;
                    if(a<5) get_light<<<1,1>>>(dls, dl_cs, a, l, cl); //raw light info
                    //if point light recal light direction
                    else{
                        get_light<<<1,1>>>(pls, pl_cs, a-5, l, cl);
...
void record_token_light(vec3 light_l[], vec3 light_color_l[], const vector<string> tokens){
    ...
    for(int i=0; i<5; i++){
        vec3 l;
        vec3 cl;
        get_light<<<1,1>>>(dls, dl_cs, i, l, cl);
        if(vec3_len(l)==0){
